...
def main(args):
    ...
    #*--------------------DIFFUSIONCLIP Optimizer and Scheduler--------------------
    ...
    scheduler_ft = torch.optim.lr_scheduler.StepLR(optim_ft, step_size=1, gamma=args.sch_gamma)
    ...
    '''
    #! Calculate mean latent vector in diffusion model
    # 1
    latent_vectors = []
    for image in images:
    ...
    # 2
    mean_latent = torch.mean(torch.stack(latent_vectors), dim=0)
    '''
    ...
    for i in pbar:
        ...
        if args.mode == "edit":
            ...
            # check : in "edit" mode, the loss is "Direct Latent Code Optimization"
