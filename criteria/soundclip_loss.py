...
class SoundCLIPLoss(torch.nn.Module):
    ...
    def __init__(self, opts):
        ...
        # TODO: "kernel_size" is originally 'stylegan_size' but we implement a diffusion, so maybe it will need to change
        self.avg_pool = torch.nn.AvgPool2d(kernel_size=256 // 32)

        self.audio_encoder = AudioEncoder()

        self.audio_encoder.load_state_dict(copyStateDict(torch.load(opts.audio_encoder_path)))
