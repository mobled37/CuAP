...
class DiffusionCLIP(object):
    ...
    def clip_finetune(self):
        # > In terms of fine-tuning, one could modify the latent or the diffusion model itself.
        # > Once the diffusion model is fine-tuned, any image from the pretrained domain can be manipulated
        # > into the image corresponding to the target text trg_txt
        print(self.args.exp)
        ...
        # ----------- Model -----------#
        #! Import here is done
        # check : this is diffusion "Pretrained model"
        ...
        if self.config.data.dataset in ["CelebA_HQ", "LSUN"]:
            # When loading a checkpoint, it is common to separate the loading of the model and optimizer parameters.
            # This is because the optimizer stores additional state information, such as the current learning rate, which should be loaded separately from the model parameter.
        ...
        #!
        # > state_dict() returns a dictionary where keys correspond to the names of parameters and values correspond to their associated tensors.
        # > init_opt_ckpt includes various information such as the current learning rate, the running average of gradients, the running average of squared gradients or etc...
        # > 쉽게 말해 state_dict()는 모델의 파라미터들을 저장하고 있는 dictionary이고, 이를 통해 모델의 파라미터들을 저장하고 불러올 수 있다.
        ...
        # > seq_inv is used to precompute the latent representations of the images.
        ...
        for mode in ['train', 'test']:
            ...
            for step, img in enumerate(loader):
                ...
                # > The expression (x0 + 1) * 0.5 is used to convert the image from [-1, 1] to [0, 1] range.
                tvu.save_image((x0 + 1) * 0.5, os.path.join(self.args.image_folder, f'{mode}_{step}_0_orig.png'))

                x = x0.clone()

                # > Diffusion Generation process using model.eval() and torch.no_grad()
                # > When a model in eval() mode, it behaves differently from train() mode.
                # > In eval() mode, batch normalization layer use the fixed population statistics rather than mini-batch statistics, and dropout layers are disabled.

                # > During the diffusion process, the model is used to generate new samples from the given input samples.
                # > Since the model parameters are already learned during the training process, they don't need to be updated during the diffusion process.
                # > Therefore, we can use torch.no_grad() to disable gradient calculation during the diffusion process.

                model.eval()
                with torch.no_grad():
                    with tqdm(total=len(seq_inv), desc=f"Inversion process {mode} {step}") as progress_bar:
                        # > seq_inv and seq_inv_next is used to calculate the time step t and t_next.
        ...
        # > For example, in the case of changing the expression of a given face image into an angry expression,
        # > We can use 'face' as a reference text and 'angry face' as a target text.
    ...
    def clip_latent_optim(self):
        ...
        for txt_idx, (src_txt, trg_txt) in enumerate(zip(self.src_txts, self.trg_txts)):
            # > wrap a tensor 'x_lat0' in an instance of the 'Parameter' class
    ...
    def edit_images_from_dataset(self):
        ...
        for mode in eval_modes:
            ...
            for step, (x0, x_id, x_lat) in enumerate(img_lat_pairs):
                ...
                with torch.no_grad():
                    x = x_lat
                    # note: x.shape = (n, c, h, w) = (1, 3, 64, 64)
                    with tqdm(total=len(seq_test), desc=f"Eval iteration") as progress_bar:
                        for i, j in zip(reversed(seq_test), reversed(seq_test_next)):
                            t = (torch.ones(n) * i).to(self.device)
                            t_next = (torch.ones(n) * j).to(self.device)
