...
audio_lists = glob("/Data/dataset/vggsound2/*.wav")
...
def func(idx):
    try:
        ...
        path = f"/Data/dataset/vggsound_curation/{name}"
...
# > parmap.map() is a drop-in replacement for the built-in map() function, but it uses multiple processes to speed up the computation.
